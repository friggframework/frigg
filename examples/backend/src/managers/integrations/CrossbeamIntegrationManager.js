...
class CrossbeamIntegrationManager extends LHIntegrationManager {
    static Config = {
        name: 'crossbeam',
        version: '1.0.0',
    ...
    async getSampleData() {
        const response = await this.targetInstance.api.getPartners();
        const slimResponse = response.partner_orgs.map((c) =>
            _.pick(c, [
                'id',
                'name',
                'url',
...
module.exports = CrossbeamIntegrationManager;
