...
router.route('/api/integrations').get(
	catchAsyncError(async (req, res) => {
		...
		const entities = await EntityManager.getEntitiesForUser(req.userManager.getUserId());
		results.entities.authorized = entities;

		// get the list of integrations
		results.integrations = await IntegrationManager.getIntegrationsForUserId(req.userManager.getUserId());
...
router.route('/api/authorize').get(
	catchAsyncError(async (req, res) => {
		const params = RouterUtil.checkRequiredParams(req.query, ['entityType', 'connectingEntityType']);
		const instance = await getEntityFromType(req, params.entityType);

		const areRequirementsValid = instance.validateAuthorizationRequirements();
		if (!areRequirementsValid) {
			throw new Error(`Error: EntityManager of type ${params.entityType} requires a valid url`);
		}
...
router.route('/api/entity/options/:credentialId').get(
	catchAsyncError(async (req, res) => {
		...
		const credential = await IntegrationManager.getCredentialById(req.params.credentialId);
		if (credential.user._id.toString() !== req.userManager.getUserId()) {
			throw Boom.forbidden('Credential does not belong to user');
		}

		const params = RouterUtil.checkRequiredParams(req.query, ['entityType']);
		const instance = await getEntityFromType(req, params.entityType);
...
router.route('/api/entity').post(
	catchAsyncError(async (req, res) => {
		...
		const credential = await IntegrationManager.getCredentialById(params.data.credential_id);
...
router.route('/api/integrations').post(
	catchAsyncError(async (req, res) => {
		const params = RouterUtil.checkRequiredParams(req.body, ['entities', 'config']);
		...
		const response = await IntegrationManager.getFormattedIntegration(integration);
		res.status(201);
		res.json(response);
	})
);
...
router.route('/api/integrations/:integrationId').patch(
	catchAsyncError(async (req, res) => {
		...
		const response = await IntegrationManager.getFormattedIntegration(integrationManagerInstance.integration);
...
router.route('/api/integrations/:integrationId').delete(
	catchAsyncError(async (req, res) => {
		...
		await IntegrationManager.deleteIntegrationForUserById(req.userManager.getUserId(), params.integrationId);
...
router.route('/api/integrations/:integrationId/config/options').get(
	catchAsyncError(async (req, res) => {
		const params = RouterUtil.checkRequiredParams(req.params, ['integrationId']);
		const integrationManagerInstance = await IntegrationManager.getInstanceFromIntegrationId(params);
...
router.route('/api/integrations/:integrationId').get(
	catchAsyncError(async (req, res) => {
		const params = RouterUtil.checkRequiredParams(req.params, ['integrationId']);
		const integration = await IntegrationManager.getIntegrationById(params.integrationId);
		// We could perhaps augment this with dynamic options? Haven't decided yet, but here may be the place
...
router.route('/api/integrations/:integrationId/test-auth').get(
	catchAsyncError(async (req, res) => {
		...
		const errors = instance.integration.messages?.errors?.filter(({ timestamp }) => timestamp >= start);
