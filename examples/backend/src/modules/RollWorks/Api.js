...
class RollWorksAPI extends OAuth2Base {
    constructor(params) {
        ...
        this.advertisable_eid = this.getParam(params, 'advertisable_eid', null);
    }
    ...
    async getTargetAccount(params) {
        const advertisableEid = this.getAdvertisableEid();
        const targetAccountId = this.getParam(params, 'targetAccountId');
        const requestData = {
    ...
    async populateTargetAccount(eid, data) {
        const advertisableEid = this.getAdvertisableEid();
        const domains = this.getParam(data, 'domains');
        const domainArr = [];
