...
describe.only('Attentive Api Class Tests', async () => {
	...
	describe('Subscriptions', async () => {
		...
		it('should get user subscriptions', async () => {
			...
			response.should.have.nested.property('subscriptionEligibilities[0].eligibility.isEligible');
		});
		it('should unsubscribe a user', async () => {
			...
			const response = await api.unsubscribeUser({ user: body.user, subscriptions: subscriptionsData });
