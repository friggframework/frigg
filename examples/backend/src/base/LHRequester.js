...
class LHRequester extends LHDelegate {
	constructor(params) {
		super(params);
		this.backOff = this.getParam(params, 'backOff', [1, 3, 10, 30, 60, 180]);
		...
		this.fetch = this.getParam(params, 'fetch', fetch);
	}

	parsedBody = async (resp) => {
		const contentType = resp.headers.get('Content-Type') || '';

		if (contentType.match(/^application\/json/) || contentType.match(/^application\/vnd.api\+json/)) {
			return resp.json();
		}
	...
	async _request(url, options, i = 0) {
		...
		if (options.query) {
			...
			for (const key in options.query) {
				queryBuild += `${encodeURIComponent(key)}=${encodeURIComponent(options.query[key])}&`;
			}
		...
		return options.returnFullRes ? response : await this.parsedBody(response);
	}
	...
	async refreshAuth() {
		throw new Error('refreshAuth not yet defined in child of LHRequester');
	}
}

module.exports = LHRequester;
