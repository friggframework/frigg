...
const LHModuleManager = require('./LHModuleManager');
const Sync = require('../models/Sync');
const Integration = require('../models/Integration');
const { debug } = require('../../utils/logger');

class LHSyncManager extends LHDelegate {
    constructor(params) {
        super(params);
        this.primaryModule = this.getParamAndVerifyType(
            params,
            'primary',
            LHModuleManager
        );
        this.secondaryModule = this.getParamAndVerifyType(
            params,
            'secondary',
            LHModuleManager
        );
        this.SyncObjectClass = this.getParamAndVerifyType(
            params,
            'syncObjectClass',
            SyncObject
        );
        this.ignoreEmptyMatchValues = this.getParam(
            params,
            'ignoreEmptyMatchValues',
            true
        );
        this.isUnidirectionalSync = this.getParam(
            params,
            'isUnidirectionalSync',
            false
        );
        this.useFirstMatchingDuplicate = this.getParam(
            params,
            'useFirstMatchingDuplicate',
            true
        );
        this.omitEmptyStringsFromData = this.getParam(
            params,
            'omitEmptyStringsFromData',
            true
        );

        this.integration = this.getParam(params, 'integration', null); // TODO Change to type validation
