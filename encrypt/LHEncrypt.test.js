  ...
  describe("Local encryption functions", () => {
    beforeAll(() => {
      process.env = {
        ...originalEnv,
        STAGE: "encryption-test",
        AES_KEY: crypto.createHash("sha256").update("secret sauce").digest(),
        AES_KEY_ID: "12345",
      };
    });
    ...
    it("automatically encrypts a secret field when using updateOne and $set", async () => {
      ...
      await Model.updateOne({ _id: doc._id }, { $set: { secret: "hijklmn" } });
    ...
    it("automatically encrypts a nested field when using updateOne", async () => {
      ...
      expect(reloadedDoc.deeply.nested).toHaveProperty("secret", "hij2lmn");
    });
    ...
    it("can use the deprecated key", async () => {
      const { Model } = createModel();
      const key = crypto.createHash("sha256").update("secret sauce").digest();
      ...
      try {
        ...
        process.env = {
          ...originalEnv,
          STAGE: "encryption-test",
          AES_KEY: crypto.createHash("sha256").update("secret 2").digest(),
          AES_KEY_ID: "67890",
