    ...
    return new Promise((resolve, reject) => {
        ...
        execFile(
            ...
            (error, stdout, stderr) => {
                ...
                if (error) {
                    console.error(
                        `Migration ${command} exited with error:`,
                        error.message
                    );
...
function validateCommand(command) {
    ...
    if (command === 'reset') {
        ...
        console.warn(
            '⚠️  WARNING: "reset" will DELETE all data and reset the database!'
        );
    }
}
...
function getSchemaPath() {
    ...
    console.log(
        'DATABASE_URL not set or database type unknown, defaulting to PostgreSQL'
    );
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (exitCode === 0) {
            ...
            throw new Error(
                `Migration ${command} failed with exit code ${exitCode}`
            );
        }
    } catch (error) {
        const duration = Date.now() - startTime;
