...
async function runPrismaGenerate(dbType, verbose = false) {
    try {
        ...
        const clientExists =
            fs.existsSync(generatedClientPath) ||
            (isLambdaEnvironment && fs.existsSync(lambdaLayerClientPath));

        if (clientExists) {
            const foundPath = fs.existsSync(generatedClientPath)
                ? generatedClientPath
                : lambdaLayerClientPath;
            if (verbose) {
                console.log(
                    chalk.gray(
                        `✓ Prisma client already generated at: ${foundPath}`
                    )
                );
            }
            if (isLambdaEnvironment) {
                if (verbose) {
                    console.log(
                        chalk.gray(
                            'Skipping generation in Lambda environment (using pre-generated client)'
                        )
                    );
                }
                return {
        ...
        if (verbose) {
            console.log(
                chalk.gray(
                    `Running: npx prisma generate --schema=${schemaPath}`
                )
            );
        }

        const output = execSync(`npx prisma generate --schema=${schemaPath}`, {
            encoding: 'utf8',
    ...
    return new Promise((resolve) => {
        try {
            ...
            if (nonInteractive) {
                console.log(
                    chalk.yellow(
                        '⚠️  Non-interactive mode: Data loss will be automatically accepted'
                    )
                );
            } else {
                console.log(
                    chalk.yellow(
                        '⚠️  Interactive mode: You may be prompted if schema changes cause data loss'
                    )
                );
            }
                ...
                if (code === 0) {
                    resolve({
                        ...
                        output: nonInteractive
                            ? stdout || 'Database push completed successfully'
 ...
 * @returns {Promise<Object>} { success: boolean, output?: string, error?: string }
 */
async function runPrismaMigrateResolve(
    migrationName,
    action = 'applied',
    verbose = false
) {
