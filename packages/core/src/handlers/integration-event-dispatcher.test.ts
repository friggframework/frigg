...
class TestIntegration extends IntegrationBase {
    ...
    async loadData() {
        (this as any).assertHydrated('loadData requires hydration');
        return { success: true };
    }
...
describe('IntegrationEventDispatcher', () => {
    const createDispatcher = () =>
        new IntegrationEventDispatcher(new TestIntegration() as any);
    ...
    describe('dispatchHttp', () => {
        ...
        it('calls initialize to register dynamic events', async () => {
            const dispatcher = createDispatcher();
            await (dispatcher as any).integrationInstance.initialize();
            const result = await dispatcher.dispatchHttp({
                event: 'DYNAMIC_EVENT',
                req: {},
                res: {},
    ...
    describe('Webhook Events', () => {
        ...
        it('should dispatch ON_WEBHOOK with job context', async () => {
            const integration = new TestIntegration({ id: '123', userId: 'user1' }) as any;
        ...
        it('should use default WEBHOOK_RECEIVED handler if not overridden', async () => {
            ...
            const handler = (dispatcher as any).findEventHandler(integration, 'WEBHOOK_RECEIVED');
