...
describe('GetIntegrationsForUser Use-Case', () => {
    ...
    let useCase: InstanceType<typeof GetIntegrationsForUser>;
    ...
    describe('happy path', () => {
        ...
        it('tracks repository operations', async () => {
            ...
            const findOperation = history.find((op: any) => op.operation === 'findByUserId');
    ...
    describe('edge cases', () => {
        it('handles user with null/undefined userId', async () => {
            const list1 = await useCase.execute(null as any);
            const list2 = await useCase.execute(undefined as any);
