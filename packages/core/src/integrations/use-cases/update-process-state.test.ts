...
describe('UpdateProcessState', () => {
    let updateProcessStateUseCase: InstanceType<typeof UpdateProcessState>;
    ...
    describe('constructor', () => {
        it('should require processRepository', () => {
            expect(() => new UpdateProcessState({} as any)).toThrow('processRepository is required');
        });

        it('should initialize with processRepository', () => {
            expect((updateProcessStateUseCase as any).processRepository).toBe(mockProcessRepository);
        });
    });
    ...
    describe('execute', () => {
        ...
        it('should throw error if processId is not a string', async () => {
            await expect(updateProcessStateUseCase.execute(123 as any, 'NEW_STATE'))
                .rejects.toThrow('processId must be a non-empty string');
        });
        ...
        it('should throw error if newState is not a string', async () => {
            await expect(updateProcessStateUseCase.execute(processId, 123 as any))
                .rejects.toThrow('newState must be a non-empty string');
        });

        it('should throw error if contextUpdates is not an object', async () => {
            await expect(updateProcessStateUseCase.execute(processId, 'NEW_STATE', 'invalid' as any))
    ...
    describe('updateStateOnly', () => {
        it('should call execute with empty context updates', async () => {
            ...
            jest.spyOn(updateProcessStateUseCase, 'execute').mockResolvedValue(updatedProcess as any);

            const result = await (updateProcessStateUseCase as any).updateStateOnly(processId, newState);
    ...
    describe('updateContextOnly', () => {
        ...
        it('should update context without changing state', async () => {
            ...
            const result = await (updateProcessStateUseCase as any).updateContextOnly(processId, contextUpdates);
        ...
        it('should throw error if process not found', async () => {
            ...
            await expect((updateProcessStateUseCase as any).updateContextOnly(processId, {}))
                .rejects.toThrow('Process not found: process-123');
        });
    });
});
