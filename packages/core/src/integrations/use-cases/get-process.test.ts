...
describe('GetProcess', () => {
    let getProcessUseCase: InstanceType<typeof GetProcess>;
    ...
    describe('constructor', () => {
        it('should require processRepository', () => {
            expect(() => new GetProcess({} as any)).toThrow('processRepository is required');
        });

        it('should initialize with processRepository', () => {
            expect((getProcessUseCase as any).processRepository).toBe(mockProcessRepository);
        });
    });
    ...
    describe('execute', () => {
        ...
        it('should throw error if processId is not a string', async () => {
            await expect(getProcessUseCase.execute(123 as any))
                .rejects.toThrow('processId must be a non-empty string');
        });
    ...
    describe('executeOrThrow', () => {
        ...
        it('should return process if found', async () => {
            ...
            const result = await (getProcessUseCase as any).executeOrThrow(processId);
        ...
        it('should throw error if process not found', async () => {
            ...
            await expect((getProcessUseCase as any).executeOrThrow(processId))
    ...
    describe('executeMany', () => {
        ...
        it('should retrieve multiple processes', async () => {
            ...
            const result = await (getProcessUseCase as any).executeMany(processIds);
        ...
        it('should throw error if processIds is not an array', async () => {
            await expect((getProcessUseCase as any).executeMany('not-an-array'))
        ...
        it('should handle mixed success and failure', async () => {
            ...
            await expect((getProcessUseCase as any).executeMany(processIds))
                .rejects.toThrow('Failed to retrieve process: Database error');
        });

        it('should handle empty array', async () => {
            const result = await (getProcessUseCase as any).executeMany([]);
