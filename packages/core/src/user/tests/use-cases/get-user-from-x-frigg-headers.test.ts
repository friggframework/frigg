...
} from '../../use-cases/get-user-from-x-frigg-headers';
import { User } from '../../user';

describe('GetUserFromXFriggHeaders', () => {
    let getUserFromXFriggHeaders: InstanceType<typeof GetUserFromXFriggHeaders>;
    ...
    describe('Validation', () => {
        it('should throw 400 error when neither appUserId nor appOrgId provided', async () => {
            await expect(
                getUserFromXFriggHeaders.execute(null as any, null as any)
            ).rejects.toThrow('x-frigg-app');

            await expect(
                getUserFromXFriggHeaders.execute(undefined as any, undefined as any)
            ).rejects.toThrow();
        });
    });
    ...
    describe('User ID Conflict Detection', () => {
        ...
        it('should throw 400 error when strictUserValidation=true and users are disconnected', async () => {
            ...
            mockUserConfig.organizationUserRequired = true;
            mockUserConfig.strictUserValidation = true;
    ...
    describe('User Config Respect', () => {
        it('should respect individualUserRequired setting', async () => {
            ...
            await getUserFromXFriggHeaders.execute('app-user-test', null as any);
