...
} from '../../use-cases/get-user-from-bearer-token';
import { TestUserRepository } from '../doubles/test-user-repository';

describe('GetUserFromBearerToken Use Case', () => {
    let userRepository: InstanceType<typeof TestUserRepository>;
    let getUserFromBearerToken: InstanceType<typeof GetUserFromBearerToken>;
    let userConfig: any;
    ...
    it('should throw an unauthorized error if the bearer token is missing', async () => {
        await expect(getUserFromBearerToken.execute(null as any)).rejects.toThrow(
    ...
    it('should throw an unauthorized error if the Session Token is not found', async () => {
        userRepository.getSessionToken = jest.fn().mockResolvedValue(null) as any;
        await expect(
