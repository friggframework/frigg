...
export class UserRepositoryPostgres extends UserRepositoryInterface {
    ...
    private _convertUserIds(user: any): UserData | null {
        if (!user) return user;
        return {
            ...user,
            id: user.id?.toString(),
    ...
    async findOrganizationUserById(userId: string): Promise<UserData | null> {
        ...
        const user = await this.prisma.user.findFirst({
            where: { id: intId, type: 'ORGANIZATION' },
        });
    ...
    async findIndividualUserById(userId: string): Promise<UserData | null> {
        const intId = this._convertId(userId);
        const user = await this.prisma.user.findFirst({
            where: { id: intId, type: 'INDIVIDUAL' },
        });
        return this._convertUserIds(user);
    }
    ...
    async createIndividualUser(params: CreateIndividualUserParams): Promise<UserData> {
        const data: Record<string, unknown> = {
            ...
            organizationId: this._convertId(params.organization || params.organizationId),
        };
    ...
    async updateIndividualUser(userId: string, updates: Partial<UserData>): Promise<UserData> {
        ...
        if (data.organizationId !== undefined) {
            data.organizationId = this._convertId(data.organizationId as string);
        }
        if ((data as any).organization !== undefined) {
            data.organizationId = this._convertId((data as any).organization);
            delete (data as any).organization;
        }
        ...
        const user = await this.prisma.user.update({ where: { id: intId }, data });
        return this._convertUserIds(user)!;
    }

    async updateOrganizationUser(userId: string, updates: Partial<UserData>): Promise<UserData> {
        const intId = this._convertId(userId);
        const user = await this.prisma.user.update({ where: { id: intId }, data: updates });
        return this._convertUserIds(user)!;
    }
    ...
    async deleteUser(userId: string): Promise<boolean> {
        try {
            const intId = this._convertId(userId);
            await this.prisma.user.delete({ where: { id: intId } });
            return true;
        } catch (error: any) {
    ...
    async linkIndividualToOrganization(individualUserId: string, organizationUserId: string): Promise<UserData> {
        ...
        const user = await this.prisma.user.update({
            where: { id: intIndividualId, type: 'INDIVIDUAL' },
            data: { organizationId: intOrganizationId },
        });
        return this._convertUserIds(user)!;
    }
}
