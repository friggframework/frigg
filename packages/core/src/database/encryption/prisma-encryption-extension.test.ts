...
describe('Prisma Encryption Extension', () => {
    ...
    beforeEach(() => {
        mockCryptor = {
            encrypt: jest
                .fn()
                .mockImplementation(
                    (value: string) => `encrypted:${value}:iv:enckey`
                ),
            decrypt: jest
                .fn()
    ...
    describe('createEncryptionExtension', () => {
        it('should create extension with valid config', () => {
            ...
            expect((extension as any).name).toBe('frigg-field-encryption');
            expect((extension as any).query).toBeDefined();
            expect((extension as any).query.$allModels).toBeDefined();
        });
        ...
        it('should return no-op extension when disabled', () => {
            ...
            const result = (extension as any)(mockClient);
        ...
        it('should throw if cryptor not provided when enabled', () => {
            expect(() => {
                createEncryptionExtension({ enabled: true } as any);
            }).toThrow('Cryptor instance required');
        });

        it('should not throw if cryptor not provided when disabled', () => {
            expect(() => {
                createEncryptionExtension({ enabled: false } as any);
            }).not.toThrow();
        });
    });
    ...
    describe('Integration with FieldEncryptionService', () => {
        it('should handle nested JSON paths correctly', async () => {
            ...
            const handlers = (extension as any).query.$allModels;
