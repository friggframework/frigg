import {
    ...
    MigrationError,
    ValidationError,
} from './run-database-migration-use-case';

describe('RunDatabaseMigrationUseCase', () => {
    let useCase: RunDatabaseMigrationUseCase;
    ...
    describe('Constructor', () => {
        it('should throw error if prismaRunner is not provided', () => {
            expect(() => new RunDatabaseMigrationUseCase({} as any)).toThrow('prismaRunner dependency is required');
        });

        it('should create instance with valid dependencies', () => {
            expect(useCase).toBeInstanceOf(RunDatabaseMigrationUseCase);
            expect((useCase as any).prismaRunner).toBe(mockPrismaRunner);
        });
    });

    describe('Parameter Validation', () => {
        it('should throw ValidationError if dbType is missing', async () => {
            await expect(useCase.execute({ stage: 'production' } as any)).rejects.toThrow(ValidationError);
            await expect(useCase.execute({ stage: 'production' } as any)).rejects.toThrow('dbType is required');
        });

        it('should throw ValidationError if dbType is not a string', async () => {
            await expect(useCase.execute({ dbType: 123, stage: 'production' } as any)).rejects.toThrow(ValidationError);
            await expect(useCase.execute({ dbType: 123, stage: 'production' } as any)).rejects.toThrow(
                'dbType must be a string'
            );
        });

        it('should throw ValidationError if stage is missing', async () => {
            await expect(useCase.execute({ dbType: 'postgresql' } as any)).rejects.toThrow(ValidationError);
            await expect(useCase.execute({ dbType: 'postgresql' } as any)).rejects.toThrow('stage is required');
        });

        it('should throw ValidationError if stage is not a string', async () => {
            await expect(useCase.execute({ dbType: 'postgresql', stage: 123 } as any)).rejects.toThrow(ValidationError);
            await expect(useCase.execute({ dbType: 'postgresql', stage: 123 } as any)).rejects.toThrow(
    ...
    describe('MongoDB Migrations', () => {
        ...
        it('should successfully run MongoDB migration', async () => {
            ...
            expect(mockPrismaRunner.runPrismaDbPush).toHaveBeenCalledWith(true, true);
            expect(mockPrismaRunner.runPrismaMigrate).not.toHaveBeenCalled();
        });
