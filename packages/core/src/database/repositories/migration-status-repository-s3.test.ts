import { MigrationStatusRepositoryS3 } from './migration-status-repository-s3';

describe('MigrationStatusRepositoryS3', () => {
    let repository: MigrationStatusRepositoryS3;
    ...
    describe('create()', () => {
        ...
        it('should generate UUID if migrationId not provided', async () => {
            ...
            expect(result.migrationId).toMatch(/^[a-f0-9-]{36}$/);
            expect(result.state).toBe('INITIALIZING');
        });
    ...
    describe('update()', () => {
        ...
        it('should merge updates with existing data', async () => {
            ...
            expect(storedData.triggeredAt).toBe('2025-10-19T12:00:00Z');
            expect(storedData.state).toBe('COMPLETED');
        });
    });
    ...
    describe('S3 Key Generation', () => {
        it('should use consistent key format', () => {
            const key = (repository as any)._buildS3Key('migration-123', 'production');
