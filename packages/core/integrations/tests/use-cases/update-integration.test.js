...
const {
    ConfigCapturingIntegration,
} = require('../doubles/config-capturing-integration');
...
describe('UpdateIntegration Use-Case', () => {
    ...
    describe('error cases', () => {
        it('throws error when integration not found', async () => {
            ...
            await expect(
                useCase.execute(nonExistentId, 'user-1', newConfig)
            ).rejects.toThrow(
        ...
        it('throws error when integration class not found', async () => {
            ...
            await expect(
                useCase.execute(record.id, 'user-1', newConfig)
            ).rejects.toThrow(
        ...
        it('throws error when user does not own integration', async () => {
            ...
            await expect(
                useCase.execute(record.id, 'different-user', newConfig)
            ).rejects.toThrow(
        ...
        it('throws error when no integration classes provided', async () => {
            ...
            await expect(
                useCaseWithoutClasses.execute(record.id, 'user-1', newConfig)
    ...
    describe('partial config update semantics (issue #514)', () => {
        ...
        it('passes existing database config to integration constructor', async () => {
            ...
            await configCapturingUseCase.execute(
                record.id,
                'user-1',
                partialUpdateConfig
            );
            ...
            const captured =
                ConfigCapturingIntegration.getCapturedOnUpdateState();
        ...
        it('allows onUpdate to merge partial config with existing config', async () => {
            ...
            const partialUpdateConfig = { type: 'config-capturing', a: 10 };
            const dto = await configCapturingUseCase.execute(
                record.id,
                'user-1',
                partialUpdateConfig
            );
