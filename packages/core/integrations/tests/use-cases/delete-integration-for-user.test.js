...
describe('DeleteIntegrationForUser Use-Case', () => {
    ...
    describe('happy path', () => {
        ...
        it('tracks delete operation', async () => {
            ...
            const deleteOperation = history.find(
    ...
    describe('error cases', () => {
        it('throws error when integration not found', async () => {
            ...
            await expect(
                ...
                `Integration with id of ${nonExistentId} does not exist`
            );
        });
    ...
    describe('edge cases', () => {
        it('handles deletion of already deleted integration', async () => {
            ...
            await expect(useCase.execute(record.id, 'user-1')).rejects.toThrow(
                `Integration with id of ${record.id} does not exist`
            );
        });
        ...
        it('handles null userId gracefully', async () => {
            ...
            await expect(useCase.execute(record.id, null)).rejects.toThrow(
                `Integration ${record.id} does not belong to User null`
            );
        });
        ...
        it('handles undefined userId gracefully', async () => {
            ...
            await expect(useCase.execute(record.id, undefined)).rejects.toThrow(
                `Integration ${record.id} does not belong to User undefined`
            );
        });
    });
});
