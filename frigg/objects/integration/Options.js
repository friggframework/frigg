...
class Options extends LHBaseClass {
    constructor(params) {
        ...
        this.module = this.getParamAndVerifyType(
            params,
            'module',
            LHModuleManager
        );
        this.integrations = this.getParamAndVerifyType(
            params,
            'integrations',
            LHModuleManager
        );
        this.isMany = Boolean(this.getParam(params, 'isMany', false));
        this.hasUserConfig = Boolean(
            this.getParam(params, 'hasUserConfig', false)
        );
        this.requiresNewEntity = Boolean(
            this.getParam(params, 'requiresNewEntity', false)
        );
        if (!params.display) {
