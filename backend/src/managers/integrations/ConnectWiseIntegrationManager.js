...
class ConnectWiseIntegrationManager extends LHIntegrationManager {
    static Config = {
        name: 'connectwise',
        version: '1.0.0',
    ...
    async processCreate(params) {
        ...
        debug('processCreate() called on the Monday Manager', params);
    ...
    async processUpdate(params) {
        const newConfig = this.getParam(params, 'config');
        const oldConfig = this.integration.config;
        if (!oldConfig.reports) this.integration.config.reports = {};
        const newReportConfig = this.getParam(newConfig, 'reports', []);
    ...
    async getSampleData() {
        const companiesResponse = await this.targetInstance.api.listCompanies();
        const slimResponse = companiesResponse.map((c) => ({
            id: c.id,
        }));
...
module.exports = ConnectWiseIntegrationManager;
