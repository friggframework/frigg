...
class MondayIntegrationManager extends LHIntegrationManager {
    static Config = {
        name: 'monday',
        version: '1.0.0',
    ...
    async getSampleData() {
        const boardsResponse = await this.targetInstance.api.getBoards();
        const slimResponse = boardsResponse.boards.map((b) => ({
            name: b.name,
        }));
...
module.exports = MondayIntegrationManager;
