...
Some Frigg integrations only work when a piece of code or configuration is deployed *outside* the Frigg runtime, on the provider's platform. A HubSpot UI Extension requires a deployed HubSpot Developer Project. A Slack app requires a published Slack App Manifest. A Salesforce-installed package requires a managed-package upload. None of this code runs in Frigg — but Frigg often acts as the *backend* the deployed thing talks to.

These outside-Frigg deliverables don't fit any of the [Extensions](./ADR-EXTENSIONS-TAXONOMY.md) types (extensions run *inside* the Frigg runtime) and aren't [Plugins](./ADR-PLUGINS.md) either. They're a distinct category.

## Decision

An **Artifact** is code or configuration that an API module helps a developer generate, but which then runs *outside* Frigg, on the target platform. Frigg's job is to:
...
|---|---|---|
| **HubSpot Developer Project** | UI Extension (React app) + serverless functions inside HubSpot Projects | `hs project add` pointer + starter template + `extensions.crmCard` (the API Module Extension that the deployed UI Extension calls into) |
...
| **Asana App Components** | Modal Forms + Widgets registered with the Asana app | Component config + `extensions.modalForm` and `extensions.widget` bridges |

### Why a distinct ADR (vs folding into API Module Extensions)

Three reasons Artifacts deserve their own concept:
...
2. **Different developer workflow.** Extensions are imported and bound. Artifacts are scaffolded (via vendor CLI) and deployed (to the provider). The Frigg CLI's role is pointing, not running.
3. **Required-or-optional varies per capability.** Some [Capabilities](./ADR-CAPABILITIES.md) (HubSpot CRM Card UI) *require* the artifact to be deployed; others can ride a different mechanism. The capability declaration needs to surface this requirement so the agent / adopter knows ahead of time.
...
The capability `crm.ui.card` declares that it requires the `developerProject` artifact to be deployed; if it isn't, the capability isn't actually available even though the code is present.
...
The artifact is *produced* outside Frigg, *deployed* outside Frigg, and *runs* outside Frigg — but it talks to Frigg via the bridge extension. Frigg is one side of the contract, not the executor.

## What artifacts are *not*

- **Not a fourth extension type.** Extensions run inside Frigg; artifacts run outside. Folding them together obscures the runtime boundary.
...
1. **Detection of "artifact actually deployed".** Frigg can know the artifact exists in the adopter's repo; can it know it's been deployed to the vendor? Some vendors expose this (HubSpot lists installed Projects); others don't. Lean: declared by the adopter (`appDefinition.artifacts.deployed: ['hubspot/developerProject']`) until vendor APIs make this introspectable.
...
3. **Multi-vendor artifacts.** Are there cases where one logical artifact spans two vendors? Probably not yet, but worth noting before something forces the question.
...
- HubSpot Developer Projects + UI Extensions documentation
- Slack App Manifest format documentation
- Salesforce managed-package documentation
- The pre-existing Asana + Frontify app structures in adopter Frigg repos (asana--frigg, frontify--frigg) — Artifacts is the formal name for what those repos already do informally
